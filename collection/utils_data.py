...
# def load_cifar100(n_examples=None, data_dir='./datasets'):
...
def load_cifar10(args, n_examples=None, data_dir='./datasets/cifar-10-clean'):
    fix_seed(args.random_seed)
    transform_chain = transforms.Compose([transforms.ToTensor()])
    dataset = datasets.CIFAR10(root=data_dir, train=True, transform=transform_chain, download=False)
    train_loader = data.DataLoader(dataset, batch_size=100, shuffle=True, num_workers=0)
    test_loader = data.DataLoader(dataset, batch_size=100, shuffle=True, num_workers=0)
    # return _load_dataset(dataset, n_examples)
    return train_loader, test_loader


def load_cifar100(args, n_examples=None, data_dir='./datasets'):
    fix_seed(args.random_seed)
    transform_chain = transforms.Compose([transforms.ToTensor()])
    dataset = datasets.CIFAR100(root=data_dir, train=True, transform=transform_chain, download=True)
    train_loader = data.DataLoader(dataset, batch_size=100, shuffle=True, num_workers=0)
    test_loader = data.DataLoader(dataset, batch_size=100, shuffle=True, num_workers=0)
    # return _load_dataset(dataset, n_examples)
    return train_loader, test_loader
...
def setup_data_loader(args, minibatch_size, data, corruption_name=None, severity=None, shuffle=True):
    ...
    if data == "imagenet":
        ...
        imagenet_trainset = torchvision.datasets.ImageFolder(
                                    root=args.data_path + 'imagenet2012/train',
                                    transform=transform_train)
        imagenet_testset = torchvision.datasets.ImageFolder(
                                    root=args.data_path + 'imagenet2012/val',
                                    transform=transform_test)
        
        # print('here')
        # print(len(imagenet_trainset))
        # print(imagenet_trainset[100:])
